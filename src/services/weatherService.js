export async function fetchWeather(city){
    const API_LAT_LONG = `https://geocoding-api.open-meteo.com/v1/search?name=${city}&count=1&language=fr&format=json`
    const req = await fetch(API_LAT_LONG)

    if(!req.ok){
        throw new Error("Erreur lors de la récupération de la latitude et longitude");
    }
